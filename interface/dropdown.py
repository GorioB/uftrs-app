...
# self.dropdown = DropDownBox(lf2.interior, label="category", toolTip="dropdown!", options=options)
class DropDownBox(Frame,object):
	def __init__(self,parent,label="Label",toolTip="None",options=["Select"],text="",**kwargs):
	...
	def initUI(self, options):
		...
		self.selectedValue = StringVar(fLower)
		self.selectedValue.set(options[0]) # default value
		self.dropdown = OptionMenu(fLower, self.selectedValue, options[0], *options)
		self.dropdown.pack(fill=BOTH, expand=1)
